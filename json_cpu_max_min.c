/*
 * json_cpu_max_min.c —— 逆向重建版源码
 *
 * 原源码已丢失；本文件按设备上 ELF 的逆向特征逐项重建：
 *   - 函数符号: main / readFileContent / wj_zr（与 ELF symtab 一致）
 *   - 错误字符串: "File does not exist or is not a regular file" /
 *     "fopen failed" / "malloc failed" / "opendir failed" /
 *     "Failed to open file" / "Usage: %s <multiplier1> <multiplier2>"
 *   - 逻辑: 遍历 /sys/devices/system/cpu/cpufreq 下 policy* 目录，
 *     读各 policy 的 cpuinfo_max_freq，按两个百分比参数乘算后
 *     写入 scaling_max_freq / scaling_min_freq；写入前 wj_zr 置零复位
 *
 * 编译（Termux）:  clang -O2 -o json_cpu_max_min json_cpu_max_min.c
 * 编译（NDK）:     aarch64-linux-android21-clang -O2 -o json_cpu_max_min json_cpu_max_min.c
 * 用法:           json_cpu_max_min <最大频率%> <最小频率%>
 */
...
/* 向 path 写入整数字符串（小工具函数）。与原 ELF 一致：写前 chmod 777 放开，写后 chmod 444 复锁 */
